...
const TRANSLATIONS = {
  de: {
    ...
    premiumPrice: "5,99€", premiumOnce: "/ einmalig",
    ...
    lang: "Sprache", liveServers: "Server", liveInstalls: "Installationen", liveTickets: "Tickets",
    statsServers: "aktive Server", statsInstalls: "Installationen", statsTickets: "bearbeitete Tickets",
  },
  en: {
    ...
    lang: "Language", liveServers: "Servers", liveInstalls: "Installs", liveTickets: "Tickets",
    statsServers: "active servers", statsInstalls: "installations", statsTickets: "tickets handled",
  }
};
...
const GLOBAL_STYLES = `
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  ::-webkit-scrollbar-thumb:hover { background: ${COLORS.blue}; }
  ...
  .hero-logo {
    ...
    border-radius: 22px;
    object-fit: cover;
    box-shadow: 0 0 60px rgba(0,229,255,0.15), 0 0 120px rgba(41,121,255,0.08);
  ...
  .hero p {
    ...
    max-width: 540px;
    margin: 0 auto 36px;
    animation: fadeInUp .8s .15s ease-out both;
  }
  .hero-btns {
    display: flex;
    gap: 16px;
  ...
  .card:hover {
    ...
    box-shadow: 0 20px 50px rgba(0,0,0,0.25), 0 0 40px rgba(0,229,255,0.05);
  }
  ...
  .server-actions a { font-size: .78rem; padding: 7px 14px; }
...
function getBotStats() {
  ...
  const tickets = servers * 12;
  return { servers, installs, tickets };
}
...
router.get("/", (req, res) => {
  ...
  const statsLabels = [txt("statsServers"), txt("statsInstalls"), txt("statsTickets")];
  const statValues = [stats.servers, stats.installs, stats.tickets];
  ...
  res.send(layout(txt("home"), `
    ...
    <div class="stats-bar">
      ${statValues.map((v, i) => `
        <div class="stat">
          <div class="stat-num" data-target="${v}" data-suffix="+">0</div>
          <div class="stat-label">${statsLabels[i]}</div>
        </div>
      `).join("")}
    </div>
    <div class="features container">
      ...
      <div class="grid">
        ...
        <div class="card"><div class="card-icon">\ud83d\ude80</div><h3>Schnell & Zuverl\u00e4ssig</h3><p>Resolvo Tool l\u00e4uft mit 99,9% Verf\u00fcgbarkeit. Dein Support steht bereit.</p></div>
      </div>
    </div>
    <script>
      (function() {
        ...
        <div class="server-card">
          ...
          <div class="server-info">
            ...
            <div class="server-actions">
              <a href="/server/${g.id}" class="btn btn-primary">${txt("configure")}</a>
            </div>
          </div>
        </div>
      `;
    ...
    <div class="content container">
      <form method="POST" action="/server/${guildId}/save" class="config-form">
        <div class="form-group">
          ...
          <select name="panel_channel_id">${sel(config.panel_channel_id, textChannels, "-- Channel w\u00e4hlen --")}</select>
        </div>
        <div class="form-group">
          <label>${txt("ticketCategory")}</label>
          <select name="ticket_category_id">${sel(config.ticket_category_id, categories, "-- Kategorie w\u00e4hlen --")}</select>
        </div>
        <div class="form-group">
          <label>${txt("transcriptChannel")}</label>
          <select name="transcript_channel_id">${sel(config.transcript_channel_id, textChannels, "-- Channel w\u00e4hlen --")}</select>
        </div>
        <div class="form-group">
          <label>${txt("supportRole")}</label>
          <select name="support_role_id">${sel(config.support_role_id, roles, "-- Rolle w\u00e4hlen --")}</select>
        </div>
        <div class="form-row">
          <div class="form-group">
