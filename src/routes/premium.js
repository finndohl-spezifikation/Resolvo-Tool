...
import { setGuildPremium, setUserPremium } from "../db.js";
...
router.get("/premium/checkout", async (req, res) => {
  ...
  try {
    ...
    const url = await createCheckoutSession(String(user), String(guild),
      `${baseUrl}/premium/success?guild=${guild}`, `${baseUrl}/premium/cancel`);
    res.redirect(url);
  } catch (err) {
...
router.post("/premium/webhook", (req, res) => {
  ...
  try {
    ...
    res.status(400).json({ error: "Ungültige Signatur" }); return;
  }
  if (event.type === "checkout.session.completed") {
    ...
    if (guildId && userId) {
      setGuildPremium(guildId, userId);
      setUserPremium(userId);
      console.log(`Premium aktiviert: Guild ${guildId}, User ${userId}`);
    }
  }
